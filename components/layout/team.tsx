...
const staff = [
  { name: "백승수", nameEn: "Baek Seung-su", nameZh: "白胜秀", nameJa: "白勝秀", role: "사무장", roleEn: "Office Manager", roleZh: "事务长", roleJa: "事務長", photo: "/team/baekss.jpg" },
  { name: "김영주", nameEn: "Kim Young-ju", nameZh: "金英珠", nameJa: "金英珠", role: "실장", roleEn: "Director", roleZh: "室长", roleJa: "室長", photo: "/team/kimyj.jpg" },
  { name: "허경", nameEn: "Heo Kyung", nameZh: "许京", nameJa: "許京", role: "실장", roleEn: "Director", roleZh: "室长", roleJa: "室長", photo: "/team/hukyung.jpg" },
]
...
export function Team({ locale = "ko" }: { locale?: string }) {
  ...
  const getKey = (locale: string, prefix: string) => {
    if (locale === "en") return `${prefix}En`
    if (locale === "zh") return `${prefix}Zh`
    if (locale === "ja") return `${prefix}Ja`
    return prefix
  }
  const roleKey = getKey(locale, "role") as keyof typeof admins[0]
  const nameKey = getKey(locale, "name") as keyof typeof admins[0]
        ...
        <div className="grid gap-3 grid-cols-3 lg:grid-cols-6 mb-4">
          {admins.map((member) => (
            <div key={member.name} className="bg-gray-50 rounded-xl p-2 text-center">
              ...
              <h4 className="font-semibold text-gray-900 text-sm">{(member[nameKey] as string) || member.name}</h4>
